...
namespace Nitroh.Windows
{
    public class PortableExecutable
    {
        ...
        private void LoadHeader()
        {
            ...
            var offset = 0;
            var dosHeader = WindowsHelper.ReadStruct<Kernel32.IMAGE_DOS_HEADER>(_headerMemory, offset);
            ...
            offset += sizeof(uint);
            var fileHeader = WindowsHelper.ReadStruct<Kernel32.IMAGE_FILE_HEADER>(_headerMemory, offset);
            if (fileHeader.Machine != Kernel32.IMAGE_FILE_MACHINE_I386) return;

            offset += Kernel32.IMAGE_SIZEOF_FILE_HEADER;
            var optHeader = WindowsHelper.ReadStruct<Kernel32.IMAGE_OPTIONAL_HEADER32>(_headerMemory, offset);

            offset = (int)optHeader.ExportTable.VirtualAddress;
            var exportDir = WindowsHelper.ReadStruct<Kernel32.IMAGE_EXPORT_DIRECTORY>(_headerMemory, offset);
            ...
            for (var index = 0; index < count; index++)
            {
                ...
                var name = WindowsHelper.ReadString(_headerMemory, namePtr);
